...
const blank = { location: '', description: '', category: 'Blocked drain', blockage: 'Partial', standing_water: false, nearby_buildings: false, latitude: '', longitude: '' };
async function api(path, options = {}) {
  const response = await fetch(`${API}${path}`, { ...options, signal: AbortSignal.timeout(15000) });
  if (!response.ok) {
    const body = await response.json().catch(() => ({}));
    throw new Error(Array.isArray(body.detail) ? body.detail.map(e => `${e.loc.at(-1)}: ${e.msg}`).join('; ') : typeof body.detail === 'string' ? body.detail : 'The server could not complete this request.');
  }
...
function App() {
  ...
  async function submit(e) {
    ...
    try {
      const saved = await api('/reports', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, latitude: Number(form.latitude), longitude: Number(form.longitude) }) });
      setForm(blank); setPage('Dashboard');
      setNotice(`Report #${saved.id} saved. ${saved.priority} review priority (${saved.score}/100).`);
      await refresh();
    } catch (e) { setError(`Submission not confirmed. Refresh the dashboard before retrying to avoid a duplicate. ${e.message}`); }
